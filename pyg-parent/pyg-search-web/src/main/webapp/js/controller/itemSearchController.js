app.controller('itemSearchController',function($scope,$location,itemSearchService){
    ...
    $scope.search=function(){
        ...
        console.log("itemSearchController.search()");
    ...
    $scope.addSearchItem=function(key,value) {
        ...
        if (key == 'category' || key == 'brand' || key=='price') {//如果点击的是分类 || 品牌 ||价格的话
            if(key == 'price'){
                console.log("if: itemSearchController.addSearchItem()  "+"key="+key+"  value="+value.text);
            }
           else {
                console.log("if: itemSearchController.addSearchItem()  "+"key="+key+"  value="+value);
            }

            $scope.searchMap[key] = value;

        } else {
            console.log("else: itemSearchController.addSearchItem()  "+"key="+key+"  value="+value);
    ...
    $scope.removeSearchItem=function(key){
        if(key=="category" || key=="brand"  || key=='price'){//如果是分类 || 品牌 ||价格的话
            $scope.searchMap[key]="";
        }else{//否则是规格
...
//判断当前页为第一页
    ...
    $scope.loadkeywords=function () {
        console.log("itemSearchController.js  keywords为"+$scope.keywords)
